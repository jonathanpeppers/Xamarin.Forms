...
(
    ...
    [string] $msbuild,
    [string] $project,
    [string] $package = 'com.xamarin.forms.helloforms',
...
if (-not $project)
{
    $project = Join-Path -Path $PSScriptRoot -ChildPath "..\..\HelloForms\HelloForms.Android\HelloForms.Android.csproj"
}
...
# Run 1 more iteration, first is discarded
for ($i = 1; $i -le $iterations + 1; $i++)
{
...
foreach ($line in $log)
{
    if ($line -match "((?<seconds>\d+)s)?(?<milliseconds>\d+)ms(\s+\(total.+\))?$")
    {
        if ($first)
        {
            $first = $false
            Write-Host $line
            Write-Host "Discarding first run ^^"
            continue
        }
...
if ($times.Count -ne 1)
{
    ...
    {
        $variance += ($time - $mean) * ($time - $mean) / ($times.Count - 1)
    }
}
$stdev = [math]::Sqrt($variance)
$stderr = $stdev / [math]::Sqrt($times.Count)
